...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (conversationID) {
      const conversation = await db.collection('conversations').findOne({ conversationID, userID });

      if (conversation) {
        // sort messages
        conversation.messages.sort((a: Message, b: Message) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
    ...
    const sortedConversations = conversations.map((conv) => ({
      ...conv,
      messages: conv.messages.sort((a: Message, b: Message) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()),
    }));

    return NextResponse.json(sortedConversations, { status: 200 });
  } catch (error) {
    console.error(error);
    return NextResponse.json({ message: 'Internal server error.' }, { status: 500 });
  }
